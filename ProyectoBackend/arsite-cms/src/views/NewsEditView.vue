...
const toggleColorPicker = (e) => {
	...
	colorPickerPos.value = { top: rect.bottom + 6, left: rect.left }
...
const loadNoticia = async () => {
	try {
		...
		const response = await api.get(`/noticias/${noticiaId.value}`)
		if (response.data.success) {
			const n = response.data.data
			form.value = {
				not_titulo: n.not_titulo || '',
				not_subtitulo: n.not_subtitulo || '',
				not_portada: null,
				not_imagen: null,
				not_video: n.not_video || '',
				not_publicacion: formatDateForInput(n.not_publicacion),
				not_estatus: n.not_estatus || 'Guardado',
			}
			if (editor.value && n.not_descripcion)
				editor.value.commands.setContent(n.not_descripcion)
			if (n.not_portada) portadaPreview.value = getImageUrl(n.not_portada)
			if (n.not_imagen) imagenPreview.value = getImageUrl(n.not_imagen)
		}
	} catch (err) {
		error.value = err.response?.data?.message || 'Error al cargar la noticia'
	} finally {
...
const deleteActiveNode = () => {
	if (editor.value?.isActive('table')) editor.value?.chain().focus().deleteTable().run()
	else editor.value?.chain().focus().deleteSelection().run()
}
...
const handleSubmit = async () => {
	...
	try {
		...
		const response = await api.post(`/noticias/${noticiaId.value}`, formData, {
		...
		} else error.value = err.response?.data?.message || 'Error al actualizar la noticia'
		window.scrollTo({ top: 0, behavior: 'smooth' })
	} finally {
		loading.value = false
	}
...
<template>
	<div class="min-h-screen bg-[#f4f6f8] p-6 md:p-8">
		...
		<!-- Loading -->
		<div v-if="loadingData" class="flex justify-center items-center py-40">
			<div class="flex flex-col items-center gap-4">
				<div
					class="animate-spin rounded-full h-14 w-14 border-4 border-amber-200 border-t-amber-600"
				></div>
				<p class="text-gray-500 text-sm font-medium">Cargando noticia...</p>
			</div>
		</div>
			...
			<div class="flex gap-8 items-start">
				...
				<div class="flex-1 min-w-0 space-y-8">
					...
					<div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
						<h2
							class="text-base font-semibold text-gray-800 mb-6 flex items-center gap-2"
						>
							<svg
						...
						<div>
							...
							<input
								...
								class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-100 outline-none transition-all text-gray-900 placeholder-gray-400"
							/>
					...
					<div
						...
						:class="{
							'fixed inset-0 z-50 rounded-none m-0 flex flex-col': isFullscreen,
						}"
					>
						<div
						...
						>
							<div
								class="flex items-center gap-0.5 pr-3 border-r border-gray-200 mr-2"
							>
								<button
									type="button"
									@click="
							...
							>
								<span class="text-xs text-red-600 font-medium mr-1"
									>{{ isActive('table') ? 'Tabla' : 'Imagen' }}:</span
								>
								<button
									type="button"
								...
								<button
									...
									@click="
										editor
											...
											.run()
									"
									class="flex items-center gap-1 px-2 py-1 text-xs text-amber-700 hover:bg-amber-100 rounded transition-all font-medium"
								>
									<svg
										class="w-3.5 h-3.5"
										fill="none"
						...
						<div class="px-6 py-3 bg-gray-50 border-t border-gray-100 flex-shrink-0">
							<p class="text-xs text-gray-400">
								<strong class="text-gray-500">Atajos:</strong> Ctrl+B (Negrita) ·
								Ctrl+I (Cursiva) · Ctrl+U (Subrayado)
							</p>
						</div>
					</div>
					...
					<div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
						...
						>
							...
							Video
							<span class="text-gray-400 font-normal text-sm ml-1">(opcional)</span>
						</h2>
						<input
						...
						<div class="p-6 space-y-5">
							<div>
								...
								<select
									...
									class="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl focus:border-amber-500 outline-none transition-all text-sm text-gray-900 bg-white appearance-none"
									style="
							...
							<div>
								...
								<input
									...
									type="datetime-local"
									class="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl focus:border-amber-500 outline-none transition-all text-sm text-gray-900"
								/>
							...
							<div class="pt-2 space-y-3 border-t border-gray-100">
								<button
									...
									class="w-full px-4 py-2.5 border-2 border-amber-200 text-amber-600 font-medium rounded-xl hover:bg-amber-50 transition-all text-sm flex items-center justify-center gap-2"
								>
									<svg
										class="w-4 h-4"
										fill="none"
								...
								<button
									...
									@click="handleSubmit"
									:disabled="loading"
									class="w-full px-4 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-600 transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
								>
									<svg
										v-if="loading"
			...
			>
				<div class="bg-white rounded-3xl shadow-2xl w-full max-w-3xl my-6 overflow-hidden">
						...
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="text-xs text-gray-500">
									Así se verá la noticia publicada
								</p>
							</div>
						</div>
						<button
					...
					<div class="overflow-y-auto max-h-[75vh]">
						...
						<div
							v-else
							class="w-full h-48 bg-gradient-to-br from-amber-50 to-orange-50 flex items-center justify-center"
						>
					...
					<div
						class="px-8 py-5 border-t border-gray-200 bg-gray-50 flex justify-end gap-3"
					>
						<button
							@click="showPreview = false"
						...
						<button
							...
							"
							:disabled="loading"
							class="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-600 transition-all text-sm shadow-lg disabled:opacity-50"
						>
							{{ loading ? 'Guardando...' : 'Guardar Cambios' }}
						</button>
					</div>
