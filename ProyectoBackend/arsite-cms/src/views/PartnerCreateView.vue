...
const savePartner = async (estatus = null) => {
	if (estatus) form.value.par_estatus = estatus
	loading.value = true
	formErrors.value = {}
	...
	if (!form.value.par_logo) {
		formErrors.value.par_logo = ['El logotipo es obligatorio']
	}
	if (!form.value.par_fecha_publicacion) {
		formErrors.value.par_fecha_publicacion = ['La fecha de publicación es obligatoria']
	}
	...
	try {
		...
		showToast(err.response?.data?.message || 'Error al crear el partner', 'error')
	} finally {
		loading.value = false
	}
}
				...
				<div class="flex items-start justify-between gap-4">
					<div>
						...
						<p class="mt-1.5 text-gray-500 text-sm max-w-md">
							Agrega una nueva empresa aliada a la sección de partners
						</p>
					</div>
					<div
					...
					>
						<span
							...
							:class="
								form.par_estatus === 'Publicado' ? 'bg-emerald-500' : 'bg-amber-400'
							"
						></span>
						<span class="text-xs font-bold text-gray-700">{{ form.par_estatus }}</span>
					</div>
				</div>
			</div>
						...
						<div class="px-6 py-6 space-y-5">
							<div class="space-y-2">
								...
								>
									...
									<button
										type="button"
										@click="form.par_estatus = 'Publicado'"
										class="flex-1 py-2 rounded-lg text-xs font-bold transition-all duration-200"
										:class="
											form.par_estatus === 'Publicado'
								...
								<div class="grid grid-cols-2 gap-2.5">
									<div class="space-y-1.5">
										...
										<input
											...
											:class="
												formErrors.par_fecha_publicacion
									...
									<div class="space-y-1.5">
										...
										<input
											...
											class="w-full px-3 py-2.5 bg-gray-50 border rounded-xl text-xs text-gray-900 outline-none transition-all duration-200"
											:class="
												formErrors.par_fecha_terminacion
													? 'border-rose-400 bg-rose-50/40'
													: 'border-gray-200 focus:bg-white focus:ring-2 focus:ring-[#312AFF]/15 focus:border-[#312AFF]'
											"
										/>
