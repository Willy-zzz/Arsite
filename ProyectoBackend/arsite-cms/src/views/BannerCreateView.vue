...
const saveBanner = async (estatus = null) => {
	if (estatus) form.value.ban_estatus = estatus
	loading.value = true
	formErrors.value = {}
	...
	try {
		...
		if (response.data.success) {
			...
			setTimeout(() => router.push('/banners'), 1400)
		}
	} catch (err) {
		if (err.response?.status === 422) formErrors.value = err.response.data.errors || {}
		showToast(err.response?.data?.message || 'Error al crear el banner', 'error')
	} finally {
		loading.value = false
	}
}
				...
				<div class="flex items-start justify-between flex-wrap gap-4">
					<div>
						...
						<p class="mt-1.5 text-gray-500 text-sm">
							Completa la información para publicar un nuevo banner en el carrusel
							principal
						</p>
					</div>
					<div
					...
					>
						<span
							...
							:class="
								form.ban_estatus === 'Publicado' ? 'bg-green-400' : 'bg-amber-400'
							"
						></span>
						<span class="text-xs font-semibold text-gray-600">{{
							form.ban_estatus === 'Publicado' ? 'Listo para publicar' : 'Borrador'
						}}</span>
					</div>
				</div>
			</div>
					...
					>
						<div class="px-7 py-5 border-b border-gray-100 flex items-center gap-3">
							...
							<div>
								...
								<p class="text-xs text-gray-400 mt-0.5">
									JPG, PNG, WEBP, SVG · Máx 5 MB · Recomendado: 1920 × 600 px
								</p>
							</div>
						</div>
							...
							<div v-if="imagePreview" class="space-y-3">
								...
								>
									...
									>
										...
										>
											{{ form.ban_imagen?.name || 'Imagen seleccionada' }}
										</span>
								...
								>
									...
									Ver preview del banner completo
								</button>
							</div>
				...
				<div class="space-y-5">
						...
						<div class="px-6 py-6 space-y-5">
							...
							<div class="space-y-2">
								...
								<div class="grid grid-cols-2 gap-2">
									...
									<button
										type="button"
										@click="form.ban_estatus = 'Publicado'"
										class="flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-medium transition-all duration-150"
										:class="
											form.ban_estatus === 'Publicado'
							...
							<div class="space-y-1.5">
								...
								/>
								<p v-if="formErrors.ban_orden" class="text-xs text-red-500">
								...
								<div class="grid grid-cols-2 gap-3">
									<div class="space-y-1">
										...
										<input
											...
											:class="
												formErrors.ban_fecha_publicacion
													? 'border-red-300 bg-red-50/60'
													: 'border-gray-200'
											"
										/>
									...
									<div class="space-y-1">
										...
										<input
											...
											class="w-full px-3 py-2.5 bg-gray-50 border rounded-xl text-xs text-gray-900 outline-none transition-all duration-200 focus:bg-white focus:ring-2 focus:ring-[#312AFF]/20 focus:border-[#312AFF]"
											:class="
												formErrors.ban_fecha_terminacion
													? 'border-red-300 bg-red-50/60'
													: 'border-gray-200'
											"
										/>
					...
					>
						...
						<p class="text-xs text-blue-600 leading-relaxed">
							Los banners en <strong>Borrador</strong> no son visibles en el sitio
							público hasta ser publicados.
						</p>
					</div>
				</div>
			</div>
					...
					<div class="relative aspect-[21/9] bg-gray-900">
						...
						>
							...
							<h2 class="text-white text-2xl lg:text-3xl font-bold mb-2 max-w-lg">
								{{
									previewBanner.ban_titulo || 'Aquí aparece el título del banner'
								}}
							</h2>
							<p class="text-white/75 text-base mb-6 max-w-md">
								{{
									previewBanner.ban_subtitulo ||
							...
							<div>
								...
								>
									{{ previewBanner.ban_texto_boton || 'Ver más' }}
								</span>
							</div>
						</div>
					</div>
