...
const allSelected = computed({
	get: () => banners.value.length > 0 && selectedBanners.value.length === banners.value.length,
	set: (value) => {
		if (value) {
			selectedBanners.value = banners.value.map((b) => b.ban_id)
		} else {
...
const hasSelectedBanners = computed(() => selectedBanners.value.length > 0)

const displayRange = computed(() => {
	if (banners.value.length === 0) return '0-0'
	const start = (pagination.value.current_page - 1) * pagination.value.per_page + 1
	const end = Math.min(start + banners.value.length - 1, pagination.value.total)
	return `${start}-${end}`
})
...
const openModal = (mode, banner = null) => {
	...
	if (mode === 'edit' && banner) {
		form.value = {
			ban_titulo: banner.ban_titulo,
			ban_subtitulo: banner.ban_subtitulo,
			ban_texto_boton: banner.ban_texto_boton,
			ban_enlace_boton: banner.ban_enlace_boton,
			ban_imagen: null,
			ban_fecha_publicacion: formatDateForInput(banner.ban_fecha_publicacion),
			ban_fecha_terminacion: formatDateForInput(banner.ban_fecha_terminacion),
			ban_orden: banner.ban_orden,
			ban_estatus: banner.ban_estatus,
		}
		imagePreview.value = banner.ban_imagen ? getImageUrl(banner.ban_imagen) : null
	}
...
const openPreview = () => {
	if (modalMode.value === 'view') {
		previewBanner.value = {
			ban_titulo: selectedBanner.value.ban_titulo,
			ban_subtitulo: selectedBanner.value.ban_subtitulo,
			ban_texto_boton: selectedBanner.value.ban_texto_boton,
			ban_enlace_boton: selectedBanner.value.ban_enlace_boton,
			ban_imagen: getImageUrl(selectedBanner.value.ban_imagen),
		}
	} else {
		previewBanner.value = {
			ban_titulo: form.value.ban_titulo,
			ban_subtitulo: form.value.ban_subtitulo,
			ban_texto_boton: form.value.ban_texto_boton,
			ban_enlace_boton: form.value.ban_enlace_boton,
			ban_imagen: imagePreview.value,
		}
	}
...
const saveBanner = async () => {
	...
	try {
		...
		const camposObligatorios = {
			ban_titulo: 'El título es obligatorio',
			ban_subtitulo: 'El subtítulo es obligatorio',
			ban_texto_boton: 'El texto del botón es obligatorio',
			ban_enlace_boton: 'El enlace URL es obligatorio',
			ban_fecha_publicacion: 'La fecha de publicación es obligatoria',
		}
		...
		if (form.value.ban_imagen instanceof File) {
			// Hay una nueva imagen seleccionada
			formData.append('ban_imagen', form.value.ban_imagen)
		}

		// ===== ENVIAR AL BACKEND =====
		formData.append('_method', 'PUT')
		const response = await api.post(`/banners/${selectedBanner.value.ban_id}`, formData, {
		...
		error.value = err.response?.data?.message || 'Error al guardar banner'
...
const handleDeleteSelected = () => {
	...
	if (selectedBanners.value.length === 1) {
		// Un solo banner: usar modal individual
		const bannerId = selectedBanners.value[0]
		const banner = banners.value.find((b) => b.ban_id === bannerId)
		...
		// Múltiples banners: usar modal masivo
...
const deleteBanner = async () => {
	...
	loading.value = true
	try {
		const response = await api.delete(`/banners/${bannerToDelete.value.ban_id}`)
		...
		error.value = err.response?.data?.message || 'Error al eliminar banner'
		showToast(error.value, 'error')
	} finally {
		loading.value = false
...
const bulkDelete = async () => {
	...
	try {
		...
		showToast(err.response?.data?.message || 'Error al eliminar banners', 'error')
	} finally {
		loading.value = false
...
const exportToExcel = async () => {
	try {
		...
		const response = await api.get('/banners/export', {
			params,
			responseType: 'blob',
		})
		...
		link.href = url
		link.download = `banners_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.xlsx`
...
const exportToPDF = async () => {
	try {
		...
		const link = document.createElement('a')
		link.href = url
		link.download = `banners_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.pdf`
		document.body.appendChild(link)
		link.click()
		document.body.removeChild(link)
		window.URL.revokeObjectURL(url)
...
const exportToCSV = () => {
	try {
		...
		const rows = banners.value.map((b) => [
			b.ban_id,
			b.ban_titulo,
			b.ban_subtitulo,
			b.ban_texto_boton,
			b.ban_enlace_boton,
			b.ban_estatus,
			b.ban_fecha_publicacion || '',
			b.ban_fecha_terminacion || '',
			b.ban_orden,
		])
		...
		const link = document.createElement('a')
		link.href = url
		link.download = `banners_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.csv`
		document.body.appendChild(link)
		link.click()
		document.body.removeChild(link)
		window.URL.revokeObjectURL(url)
...
const handleDrop = async (event, dropIndex) => {
	...
	// Crear una copia del array de banners para manipular
	const updatedBanners = [...banners.value]

	// Remover el banner que estamos arrastrando
	const [draggedBanner] = updatedBanners.splice(draggedIndex.value, 1)
	...
	const reorderedData = updatedBanners.map((banner, index) => ({
		id: banner.ban_id,
	...
	try {
		...
		const response = await api.put('/banners/update-order', {
			banners: reorderedData,
		})

		if (response.data.success) {
			// Actualizar los valores de ban_orden en cada banner
			updatedBanners.forEach((banner, index) => {
				banner.ban_orden = index + 1
			})

			showToast('Orden actualizado exitosamente', 'success')
		}
	} catch (err) {
		// Si falla, recargar los banners para revertir el cambio visual
...
<template>
	<div class="min-h-screen bg-[#f4f6f8] p-4 md:p-6">
		...
		<div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
			...
			<div class="bg-[#f6f6f6] border-b border-gray-200 px-6 py-4">
				<h3 class="text-xl font-semibold text-[#1c2321]">
					Administración del carrusel de banners
				</h3>
			</div>
			...
			<div class="px-6 pb-6">
				...
				>
					<h4 class="text-lg font-bold tracking-tight">Banners</h4>
				...
				<div class="overflow-x-auto border border-t-0 border-gray-300 rounded-b-xl">
					<table class="w-full border-collapse table-fixed">
						...
						<thead>
							<tr class="bg-[#f8f9fa] border-b border-gray-300">
								...
								<th
									...
									@click="setSorting('ban_fecha_publicacion')"
								>
									<div class="flex items-center gap-1">
										...
										<svg
											class="w-3 h-3 transition-all"
											:class="
												filters.sort_by === 'ban_fecha_publicacion'
													? 'text-[#312AFF]'
													: 'text-gray-400 group-hover:text-gray-600'
											"
											fill="none"
								...
								<th
									class="px-2 py-3 text-left text-[12px] font-bold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-200 transition-colors group"
									@click="setSorting('ban_fecha_terminacion')"
								>
									<div class="flex items-center gap-1">
										Terminación
										<svg
											class="w-3 h-3 transition-all"
											:class="
												filters.sort_by === 'ban_fecha_terminacion'
													? 'text-[#312AFF]'
													: 'text-gray-400 group-hover:text-gray-600'
											"
											fill="none"
						...
						<tbody class="divide-y divide-gray-200 bg-white">
							...
							<tr v-else-if="error && banners.length === 0">
							...
							>
								...
								<td
									class="px-2 py-3 text-center text-xs font-semibold text-gray-900"
								>
									<div class="truncate" :title="banner.ban_titulo">
										{{ banner.ban_titulo }}
									</div>
								</td>
								<td class="px-2 py-3 text-center">
								...
								<td class="px-2 py-3 text-center">
									<span
										:class="getStatusColor(banner.ban_estatus)"
								...
								>
									{{ formatDate(banner.ban_fecha_publicacion) }}
								</td>
								<td
									class="px-2 py-3 text-center text-[12px] text-gray-600 whitespace-nowrap"
								>
									{{ formatDate(banner.ban_fecha_terminacion) }}
								</td>
								<td class="px-2 py-3 text-center">
									<button
										@click="openModal('view', banner)"
				...
				>
					<div>
						<h3 class="text-xl font-bold text-white">
							{{ modalMode === 'edit' ? 'Editar Banner' : 'Detalles del Banner' }}
						</h3>
						<p class="text-indigo-100 text-xs mt-0.5">
							Personaliza tu banner del carrusel principal
						</p>
					</div>
					<button
						@click="closeModal"
							...
							>
								...
								<div class="group">
									...
									<textarea
										...
										placeholder="Describe brevemente tu banner..."
									></textarea>
									<div class="flex justify-between items-center mt-1.5">
										<p
											v-if="formErrors.ban_subtitulo"
											class="text-red-500 text-sm"
										>
								...
								<div class="grid grid-cols-2 gap-4">
									...
									<div>
										...
										<input
											v-model="form.ban_texto_boton"
											type="text"
											maxlength="50"
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.ban_texto_boton,
											}"
											placeholder="Ej: Conocer más"
										/>
										<p
											v-if="formErrors.ban_texto_boton"
											class="text-red-500 text-sm mt-1"
										>
											{{ formErrors.ban_texto_boton[0] }}
										</p>
									</div>
									...
									<div>
										...
										<input
											v-model="form.ban_enlace_boton"
											type="url"
											maxlength="255"
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.ban_enlace_boton,
											}"
											placeholder="https://ejemplo.com"
										/>
										<p
											v-if="formErrors.ban_enlace_boton"
											class="text-red-500 text-sm mt-1"
										>
											{{ formErrors.ban_enlace_boton[0] }}
										</p>
									</div>
								</div>
							</div>

							<!-- ========== TAB 2: IMAGEN Y DISEÁ‘O ========== -->
							<div
							...
							>
								...
								<div>
									...
									<div
										...
										:class="{ 'border-red-500': formErrors.ban_imagen }"
									>
									...
									<p
										v-if="formErrors.ban_imagen"
										class="text-red-500 text-sm mt-2"
									>
										{{ formErrors.ban_imagen[0] }}
									</p>
								</div>
							</div>
							...
							>
								...
								<div class="grid grid-cols-2 gap-4">
									<div>
										...
										<input
											v-model="form.ban_fecha_publicacion"
											type="date"
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.ban_fecha_publicacion,
											}"
										/>
										<p
											v-if="formErrors.ban_fecha_publicacion"
											class="text-red-500 text-sm mt-1"
										>
											{{ formErrors.ban_fecha_publicacion[0] }}
										</p>
									</div>
									...
									<div>
										...
										<input
											v-model="form.ban_fecha_terminacion"
											type="date"
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.ban_fecha_terminacion,
											}"
										/>
										<p
											v-if="formErrors.ban_fecha_terminacion"
											class="text-red-500 text-sm mt-1"
										>
											{{ formErrors.ban_fecha_terminacion[0] }}
										</p>
									</div>
								</div>
								...
								<div class="grid grid-cols-2 gap-4 pb-2.5">
									<div>
										...
										<input
											v-model.number="form.ban_orden"
											type="number"
											min="0"
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{ 'border-red-500': formErrors.ban_orden }"
										/>
										<p
											v-if="formErrors.ban_orden"
											class="text-red-500 text-sm mt-1"
										>
											{{ formErrors.ban_orden[0] }}
										</p>
									</div>
									...
									<div>
										...
										<select
											...
											:class="{ 'border-red-500': formErrors.ban_estatus }"
											style="
										...
										<p
											v-if="formErrors.ban_estatus"
											class="text-red-500 text-sm mt-1"
										>
											{{ formErrors.ban_estatus[0] }}
										</p>
									</div>
								</div>
									...
									<div class="flex gap-3">
										...
										<div class="text-sm text-blue-700">
											...
											<p class="text-blue-600">
												El banner se publicará y ocultará automáticamente
							...
							<div class="flex-1 overflow-y-auto p-8 space-y-10">
								...
								>
									...
									<div class="flex justify-center">
										...
										>
											<img
												:src="getImageUrl(selectedBanner.ban_imagen)"
												:alt="selectedBanner.ban_titulo"
									...
									<div class="p-6 space-y-6">
										...
										<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
											...
											<div class="group">
												...
												>
													<p class="text-gray-900 font-medium">
														{{ selectedBanner.ban_texto_boton }}
													</p>
												</div>
											</div>
										...
										<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
											...
											<div class="group">
												...
												>
													<p class="text-gray-900 font-medium">
														{{
															formatDate(
																selectedBanner.ban_fecha_publicacion,
															)
														}}
													</p>
												</div>
											...
											<div class="group">
												...
												>
													<p class="text-gray-900 font-medium">
														{{
															selectedBanner.ban_fecha_terminacion
																? formatDate(
																		selectedBanner.ban_fecha_terminacion,
																	)
											...
											<div class="flex items-center gap-3">
												<span
													:class="[
														...
														selectedBanner.ban_estatus === 'Publicado'
															? 'bg-emerald-50 text-emerald-700 border-emerald-200'
															: selectedBanner.ban_estatus ===
																  'Guardado'
									...
									<div class="grid grid-cols-1 md:grid-cols-2 gap-5 text-sm">
										<div class="flex items-center gap-2">
											...
											<span class="font-semibold text-gray-900">{{
												selectedBanner.usuario?.name || 'N/A'
											}}</span>
										</div>
										<div class="flex items-center gap-2">
											<svg
										...
										<div class="flex items-center gap-2">
											...
											<span class="font-medium text-gray-900">{{
												formatDate(selectedBanner.updated_at)
											}}</span>
										</div>
									</div>
								</div>
		...
		<!-- Modal de vista previa del banner -->
		<div
		...
		>
			...
			>
				...
				>
					<div>
						<h3 class="text-xl font-bold text-white">Vista Previa del Banner</h3>
				...
				<div class="flex-1 overflow-y-auto">
					<div class="p-6 bg-gray-100">
						...
						>
							<!-- Imagen de fondo -->
							<img
								v-if="previewBanner?.ban_imagen"
								:src="previewBanner.ban_imagen"
								alt="Banner preview"
								class="w-full h-full object-cover"
							/>
							...
							>
								<div class="p-8 md:p-12 max-w-2xl animate-slideInLeft">
									...
									>
										{{ previewBanner?.ban_titulo || 'Título del banner' }}
									</h2>
									<p
										class="text-lg md:text-xl text-white/95 mb-6 md:mb-8 drop-shadow-md leading-relaxed"
									>
										{{ previewBanner?.ban_subtitulo || 'Subtítulo del banner' }}
									</p>
									<button
									...
									>
										{{ previewBanner?.ban_texto_boton || 'Texto del botón' }}
									</button>
								</div>
							</div>
		...
		>
			...
			>
				...
				<div class="bg-gradient-to-br from-red-500 to-red-600 px-6 py-4 text-center">
					...
					<h3 class="text-xl font-bold text-white">¿Eliminar Banner?</h3>
				</div>
				...
				<div class="p-8">
					<p class="text-gray-700 text-center mb-6 text-lg pb-2">
						...
						<span class="font-bold text-red-600"
							>{{ selectedBanners.length }} banner(s)</span
						>
						seleccionado(s).
					</p>
					...
					<div class="bg-red-50 border-l-4 border-red-500 p-2 rounded-r-xl mb-6">
						<div class="flex gap-3">
							...
							<div class="text-sm text-red-800">
								...
								<p>
									Todos los banners seleccionados serán eliminados permanentemente
									del sistema.
								</p>
							</div>
						</div>
