...
const allSelected = computed({
	get: () =>
		servicios.value.length > 0 && selectedServicios.value.length === servicios.value.length,
	set: (value) => {
		if (value) {
			selectedServicios.value = servicios.value.map((s) => s.ser_id)
		} else {
...
const hasSelectedServicios = computed(() => selectedServicios.value.length > 0)

const displayRange = computed(() => {
	if (servicios.value.length === 0) return '0-0'
	const start = (pagination.value.current_page - 1) * pagination.value.per_page + 1
	const end = Math.min(start + servicios.value.length - 1, pagination.value.total)
	return `${start}-${end}`
})
...
const openModal = (mode, servicio = null) => {
	...
	if (mode === 'edit' && servicio) {
		form.value = {
			ser_titulo: servicio.ser_titulo,
			ser_descripcion: servicio.ser_descripcion,
			ser_imagen: null,
			ser_orden: servicio.ser_orden,
			ser_fecha_publicacion: formatDateForInput(servicio.ser_fecha_publicacion),
			ser_fecha_terminacion: formatDateForInput(servicio.ser_fecha_terminacion),
			ser_estatus: servicio.ser_estatus,
		}
		imagePreview.value = servicio.ser_imagen ? getImageUrl(servicio.ser_imagen) : null
	}
...
const saveServicio = async () => {
	...
	try {
		...
		const response = await api.post(`/servicios/${selectedServicio.value.ser_id}`, formData, {
		...
		error.value = err.response?.data?.message || 'Error al guardar servicio'
...
const handleDeleteSelected = () => {
	if (selectedServicios.value.length === 0) return

	if (selectedServicios.value.length === 1) {
		const servicioId = selectedServicios.value[0]
		const servicio = servicios.value.find((s) => s.ser_id === servicioId)
		if (servicio) {
			servicioToDelete.value = servicio
			showDeleteConfirm.value = true
		}
...
const deleteServicio = async () => {
	...
	loading.value = true
	try {
		const response = await api.delete(`/servicios/${servicioToDelete.value.ser_id}`)
		...
		error.value = err.response?.data?.message || 'Error al eliminar servicio'
		showToast(error.value, 'error')
	} finally {
		loading.value = false
...
const bulkDelete = async () => {
	...
	loading.value = true
	try {
		const response = await api.delete('/servicios/bulk-delete', {
			data: { ids: selectedServicios.value },
		})
		...
		showToast(err.response?.data?.message || 'Error al eliminar servicios', 'error')
	} finally {
		loading.value = false
...
const exportToExcel = async () => {
	try {
		...
		const response = await api.get('/servicios/export', {
			params,
			responseType: 'blob',
		})
		...
		link.href = url
		link.download = `servicios_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.xlsx`
...
const exportToPDF = async () => {
	try {
		...
		const link = document.createElement('a')
		link.href = url
		link.download = `servicios_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.pdf`
		document.body.appendChild(link)
		link.click()
		document.body.removeChild(link)
		window.URL.revokeObjectURL(url)
...
const exportToCSV = () => {
	try {
		...
		const rows = servicios.value.map((s) => [
			s.ser_id,
			s.ser_titulo,
			s.ser_descripcion,
			s.ser_estatus,
			s.ser_fecha_publicacion || '',
			s.ser_fecha_terminacion || '',
			s.ser_orden,
		])
		...
		const link = document.createElement('a')
		link.href = url
		link.download = `servicios_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.csv`
		document.body.appendChild(link)
		link.click()
		document.body.removeChild(link)
		window.URL.revokeObjectURL(url)
...
const handleDrop = async (event, dropIndex) => {
	...
	const updatedServicios = [...servicios.value]
	const [draggedServicio] = updatedServicios.splice(draggedIndex.value, 1)
	updatedServicios.splice(dropIndex, 0, draggedServicio)

	// Actualizar visualmente de inmediato
	servicios.value = updatedServicios

	const reorderedData = updatedServicios.map((servicio, index) => ({
		id: servicio.ser_id,
		orden: index + 1,
	}))

	try {
		const response = await api.put('/servicios/update-order', {
			servicios: reorderedData,
		})

		if (response.data.success) {
			updatedServicios.forEach((servicio, index) => {
				servicio.ser_orden = index + 1
			})
	...
	<div class="min-h-screen bg-[#f4f6f8] p-4 md:p-6">
		...
		<div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
			...
			<div class="bg-[#f6f6f6] border-b border-gray-200 px-6 py-4">
				<h3 class="text-xl font-semibold text-[#1c2321]">Administración de Servicios</h3>
			</div>
			...
			<div class="px-6 pb-6">
				...
				>
					<h4 class="text-lg font-bold tracking-tight">Servicios</h4>
					...
					<table class="w-full border-collapse table-fixed">
						...
						<thead>
							<tr class="bg-[#f8f9fa] border-b border-gray-300">
								...
								<th
									class="px-2 py-3 text-center text-[12px] font-bold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors"
									@click="setSorting('ser_fecha_publicacion')"
								>
									<div class="flex items-center justify-center gap-1">
										...
										>
											<path
												v-if="
													filters.sort_by === 'ser_fecha_publicacion' &&
													filters.sort_direction === 'asc'
												"
												stroke-linecap="round"
												stroke-linejoin="round"
						...
						<tbody class="divide-y divide-gray-200 bg-white">
							...
							<tr v-else-if="error && servicios.length === 0">
							...
							>
								...
								<td
									class="px-2 py-3 text-center text-xs font-semibold text-gray-900"
								>
									<div class="truncate" :title="servicio.ser_titulo">
										{{ servicio.ser_titulo }}
									</div>
								</td>
								<td class="px-2 py-3 text-center">
		...
		>
			<div
				class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full overflow-hidden animate-slideUp max-h-[90vh] flex flex-col"
			>
				<!-- HEADER -->
				<div
				...
				>
					<div>
						<h3 class="text-xl font-bold text-white">Editar Servicio</h3>
				...
				<div class="p-8 overflow-y-auto flex-1">
					<form @submit.prevent="saveServicio" class="space-y-6">
						<transition name="tab-content" mode="out-in">
							...
							>
								...
								<div class="group">
									...
									<input
										...
										:class="{ 'border-red-500': formErrors.ser_titulo }"
									/>
									<div class="flex justify-between items-center mt-1.5">
										...
										<p class="text-xs text-gray-500 ml-auto">
											{{ form.ser_titulo?.length || 0 }}/100 caracteres
										</p>
									</div>
								</div>
								...
								<div class="group">
									...
									<textarea
										...
										:class="{ 'border-red-500': formErrors.ser_descripcion }"
									></textarea>
									<p
								...
								<div>
									...
									<select
										...
										:class="{ 'border-red-500': formErrors.ser_estatus }"
										style="
									...
									<p
										v-if="formErrors.ser_estatus"
										class="text-red-500 text-sm mt-1"
									>
										{{ formErrors.ser_estatus[0] }}
									</p>
								</div>
							</div>
								...
								<div>
									...
									<div v-if="imagePreview" class="mb-4 relative group">
										...
										>
											...
											<div
												class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl"
											/>
											<button
												type="button"
							...
							>
								...
								<div class="grid grid-cols-2 gap-4">
									<div>
										...
										<input
											...
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.ser_fecha_publicacion,
											}"
										/>
										<p
									...
									<div>
										...
										<input
											...
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.ser_fecha_terminacion,
											}"
										/>
										<p
								...
								<div>
									...
									<input
										v-model.number="form.ser_orden"
										type="number"
										min="0"
										class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
										:class="{ 'border-red-500': formErrors.ser_orden }"
									/>
									<p
										v-if="formErrors.ser_orden"
										class="text-red-500 text-sm mt-1"
									>
										{{ formErrors.ser_orden[0] }}
									</p>
								</div>
									...
									<div class="flex gap-3">
										...
										<div class="text-sm text-blue-700">
											...
											<p class="text-blue-600">
												El servicio se publicará y ocultará automáticamente
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="text-white/80 text-sm">Información del servicio</p>
							</div>
						</div>
						<button
							@click="closeModal"
				...
				<div class="flex-1 overflow-y-auto p-8 space-y-8">
					...
					>
						<div class="flex items-center gap-2 mb-5">
							...
							<h4 class="font-semibold text-gray-900">Imagen del Servicio</h4>
						</div>
						<div class="flex justify-center">
							<div class="bg-white rounded-xl overflow-hidden shadow-md w-full">
								<img
									v-if="selectedServicio?.ser_imagen"
									:src="getImageUrl(selectedServicio.ser_imagen)"
									:alt="selectedServicio.ser_titulo"
						...
						<div class="p-6 space-y-5">
							...
							<div>
								...
								<div class="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
									<p class="text-gray-700 whitespace-pre-wrap leading-relaxed">
										{{ selectedServicio?.ser_descripcion }}
									</p>
								</div>
							</div>
							...
							<div class="grid grid-cols-2 gap-4">
								<div>
									...
									>
										<span
											:class="getStatusColor(selectedServicio?.ser_estatus)"
											class="inline-block px-3 py-1 text-xs font-bold uppercase rounded-full border"
										>
											{{ selectedServicio?.ser_estatus }}
										</span>
									</div>
								</div>
								<div>
					...
					<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
						...
						<div class="p-6 grid grid-cols-2 gap-4">
							<div>
								...
								<div class="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
									<p class="text-gray-900 font-medium">
										{{ formatDate(selectedServicio?.ser_fecha_publicacion) }}
									</p>
								</div>
							</div>
							<div>
								...
								<div class="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
									<p class="text-gray-900 font-medium">
										{{ formatDate(selectedServicio?.ser_fecha_terminacion) }}
									</p>
								</div>
							</div>
						</div>
						...
						<div class="p-6 grid grid-cols-1 gap-3 text-sm">
							...
							<div class="flex items-center gap-2">
								...
								<span class="font-medium text-gray-900">{{
									selectedServicio?.user?.usu_nombre ?? '—'
								}}</span>
							</div>
							<div class="flex items-center gap-2">
								<svg
								...
								<span class="font-medium text-gray-900">{{
									formatDate(selectedServicio?.created_at)
								}}</span>
							</div>
							<div class="flex items-center gap-2">
								<svg
								...
								<span class="font-medium text-gray-900">{{
									formatDate(selectedServicio?.updated_at)
								}}</span>
							</div>
						</div>
					</div>
		...
		>
			...
			>
				<div class="bg-gradient-to-br from-red-500 to-red-600 px-6 py-4 text-center">
					...
					<h3 class="text-xl font-bold text-white">¿Eliminar Servicio?</h3>
				</div>
				<div class="p-8">
				...
				<div class="p-8">
					<p class="text-gray-700 text-center mb-6 text-lg pb-2">
						...
						<span class="font-bold text-red-600"
							>{{ selectedServicios.length }} servicio(s)</span
						>
						seleccionado(s).
					</p>
					<div class="bg-red-50 border-l-4 border-red-500 p-2 rounded-r-xl mb-6">
						<div class="flex gap-3">
							...
							<div class="text-sm text-red-800">
								...
								<p>
									Todos los servicios seleccionados serán eliminados
