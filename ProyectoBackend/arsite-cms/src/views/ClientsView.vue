...
const allSelected = computed({
	get: () => clientes.value.length > 0 && selectedClientes.value.length === clientes.value.length,
	set: (value) => {
		if (value) {
			selectedClientes.value = clientes.value.map((c) => c.cli_id)
		} else {
...
const hasSelectedClientes = computed(() => selectedClientes.value.length > 0)

const displayRange = computed(() => {
	if (clientes.value.length === 0) return '0-0'
	const start = (pagination.value.current_page - 1) * pagination.value.per_page + 1
	const end = Math.min(start + clientes.value.length - 1, pagination.value.total)
	return `${start}-${end}`
})
...
const openModal = (mode, cliente = null) => {
	...
	if (mode === 'edit' && cliente) {
		form.value = {
			...
			cli_logo: null,
			cli_orden: cliente.cli_orden,
			cli_fecha_publicacion: formatDateForInput(cliente.cli_fecha_publicacion),
			cli_fecha_terminacion: formatDateForInput(cliente.cli_fecha_terminacion),
			cli_estatus: cliente.cli_estatus,
		}
		imagePreview.value = cliente.cli_logo ? getImageUrl(cliente.cli_logo) : null
	}
...
const saveCliente = async () => {
	...
	try {
		...
		if (!imagePreview.value) {
			formErrors.value.cli_logo = ['El logo es obligatorio']
		}
		...
		const response = await api.post(`/clientes/${selectedCliente.value.cli_id}`, formData, {
		...
		error.value = err.response?.data?.message || 'Error al guardar cliente'
...
const handleDeleteSelected = () => {
	if (selectedClientes.value.length === 0) return

	if (selectedClientes.value.length === 1) {
		const clienteId = selectedClientes.value[0]
		const cliente = clientes.value.find((c) => c.cli_id === clienteId)
		if (cliente) {
			clienteToDelete.value = cliente
			showDeleteConfirm.value = true
		}
...
const deleteCliente = async () => {
	...
	loading.value = true
	try {
		const response = await api.delete(`/clientes/${clienteToDelete.value.cli_id}`)
...
const bulkDelete = async () => {
	...
	loading.value = true
	try {
		const response = await api.delete('/clientes/bulk-delete', {
			data: { ids: selectedClientes.value },
		})
		...
		showToast(err.response?.data?.message || 'Error al eliminar clientes', 'error')
	} finally {
		loading.value = false
...
const exportToExcel = async () => {
	try {
		...
		link.href = url
		link.download = `clientes_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.xlsx`
...
const exportToPDF = async () => {
	try {
		...
		const link = document.createElement('a')
		link.href = url
		link.download = `clientes_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.pdf`
		document.body.appendChild(link)
		link.click()
		document.body.removeChild(link)
		window.URL.revokeObjectURL(url)
...
const exportToCSV = () => {
	try {
		...
		const rows = clientes.value.map((c) => [
			...
			c.cli_estatus,
			c.cli_orden,
			c.cli_fecha_publicacion || '',
			c.cli_fecha_terminacion || '',
		])
		...
		const link = document.createElement('a')
		link.href = url
		link.download = `clientes_${new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Mexico_City' })}.csv`
		document.body.appendChild(link)
		link.click()
		document.body.removeChild(link)
		window.URL.revokeObjectURL(url)
...
const handleDrop = async (event, dropIndex) => {
	...
	const updatedClientes = [...clientes.value]
	const [draggedCliente] = updatedClientes.splice(draggedIndex.value, 1)
	updatedClientes.splice(dropIndex, 0, draggedCliente)

	// Actualizar visualmente de inmediato
	clientes.value = updatedClientes

	const reorderedData = updatedClientes.map((cliente, index) => ({
		id: cliente.cli_id,
		orden: index + 1,
	}))

	try {
		const response = await api.put('/clientes/update-order', {
			clientes: reorderedData,
		})

		if (response.data.success) {
			updatedClientes.forEach((cliente, index) => {
				cliente.cli_orden = index + 1
			})
	...
	<div class="min-h-screen bg-[#f4f6f8] p-4 md:p-6">
		...
		<div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
			...
			<div class="bg-[#f6f6f6] border-b border-gray-200 px-6 py-4">
				<h3 class="text-xl font-semibold text-[#1c2321]">Administración de Clientes</h3>
			</div>
			...
			<div class="px-6 pb-6">
				...
				>
					<h4 class="text-lg font-bold tracking-tight">Clientes</h4>
					...
					<table class="w-full border-collapse table-fixed">
						...
						<thead>
							<tr class="bg-[#f8f9fa] border-b border-gray-300">
								...
								<th
									class="px-2 py-3 text-center text-[12px] font-bold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors"
									@click="setSorting('cli_fecha_publicacion')"
								>
									<div class="flex items-center justify-center gap-1">
										...
										>
											<path
												v-if="
													filters.sort_by === 'cli_fecha_publicacion' &&
													filters.sort_direction === 'asc'
												"
												stroke-linecap="round"
												stroke-linejoin="round"
						...
						<tbody class="divide-y divide-gray-200 bg-white">
							...
							<tr v-else-if="error && clientes.length === 0">
							...
							>
								...
								<td
									class="px-2 py-3 text-center text-xs font-semibold text-gray-900"
								>
									<div class="truncate" :title="cliente.cli_nombre">
										{{ cliente.cli_nombre }}
									</div>
								</td>
								<td class="px-2 py-3 text-center">
								...
								<td class="px-2 py-3 text-center">
									<span
										:class="getStatusColor(cliente.cli_estatus)"
								...
								>
									{{ formatDate(cliente.cli_fecha_publicacion) }}
								</td>
								<td
									class="px-2 py-3 text-center text-[12px] text-gray-600 whitespace-nowrap"
								>
									{{ formatDate(cliente.cli_fecha_terminacion) }}
								</td>
								<td class="px-2 py-3 text-center">
									<button
										@click="openModal('view', cliente)"
						...
						<transition name="tab-content" mode="out-in">
							...
							>
								...
								<div class="group">
									...
									<input
										...
										:class="{ 'border-red-500': formErrors.cli_nombre }"
									/>
									<div class="flex justify-between items-center mt-1.5">
										...
										<p class="text-xs text-gray-500 ml-auto">
											{{ form.cli_nombre?.length || 0 }}/50 caracteres
										</p>
									</div>
								</div>
								...
								<div>
									...
									<select
										...
										:class="{ 'border-red-500': formErrors.cli_estatus }"
										style="
									...
									<p
										v-if="formErrors.cli_estatus"
										class="text-red-500 text-sm mt-1"
									>
										{{ formErrors.cli_estatus[0] }}
									</p>
								</div>
							</div>
								...
								<div>
									...
									<div
										...
										:class="[
											...
											formErrors.cli_logo ? 'border-red-500' : '',
										]"
									>
									...
									<p v-if="formErrors.cli_logo" class="text-red-500 text-sm mt-2">
							...
							>
								...
								<div class="grid grid-cols-2 gap-4">
									<div>
										...
										<input
											...
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.cli_fecha_publicacion,
											}"
										/>
										<p
									...
									<div>
										...
										<input
											...
											class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
											:class="{
												'border-red-500': formErrors.cli_fecha_terminacion,
											}"
										/>
										<p
								...
								<div>
									...
									<input
										v-model.number="form.cli_orden"
										type="number"
										min="0"
										class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all duration-200 outline-none text-gray-900"
										:class="{ 'border-red-500': formErrors.cli_orden }"
									/>
									<p
										v-if="formErrors.cli_orden"
										class="text-red-500 text-sm mt-1"
									>
										{{ formErrors.cli_orden[0] }}
									</p>
								</div>
									...
									<div class="flex gap-3">
										...
										<div class="text-sm text-blue-700">
											...
											<p class="text-blue-600">
												El cliente se publicará y ocultará automáticamente
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="text-white/80 text-sm">Información del cliente</p>
							</div>
						</div>
						<button
							@click="closeModal"
				...
				<div class="flex-1 overflow-y-auto p-8 space-y-8">
					...
					>
						<div class="flex items-center gap-2 mb-5">
							...
							<h4 class="font-semibold text-gray-900">Logo del Cliente</h4>
						</div>
						...
						<div class="flex justify-center">
							...
							>
								<img
									v-if="selectedCliente?.cli_logo"
									:src="getImageUrl(selectedCliente.cli_logo)"
									:alt="selectedCliente.cli_nombre"
									class="max-h-32 max-w-full object-contain"
								/>
					...
					<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
						...
						<div class="p-6 grid grid-cols-2 gap-4">
							<div>
								...
								<div class="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
									<p class="text-gray-900 font-medium">
										{{ formatDate(selectedCliente?.cli_fecha_publicacion) }}
									</p>
								</div>
							</div>
							<div>
								...
								<div class="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
									<p class="text-gray-900 font-medium">
										{{ formatDate(selectedCliente?.cli_fecha_terminacion) }}
									</p>
								</div>
							</div>
						</div>
						...
						<div class="p-6 grid grid-cols-1 gap-3 text-sm">
							...
							<div class="flex items-center gap-2">
								...
								<span class="font-medium text-gray-900">{{
									selectedCliente?.user?.usu_nombre ?? '—'
								}}</span>
							</div>
							<div class="flex items-center gap-2">
								<svg
							...
							<div class="flex items-center gap-2">
								...
								<span class="font-medium text-gray-900">{{
									formatDate(selectedCliente?.updated_at)
								}}</span>
							</div>
						</div>
					</div>
		...
		>
			...
			>
				<div class="bg-gradient-to-br from-red-500 to-red-600 px-6 py-4 text-center">
					...
					<h3 class="text-xl font-bold text-white">¿Eliminar Cliente?</h3>
				</div>
				<div class="p-8">
				...
				<div class="p-8">
					<p class="text-gray-700 text-center mb-6 text-lg pb-2">
						...
						<span class="font-bold text-red-600"
							>{{ selectedClientes.length }} cliente(s)</span
						>
						seleccionado(s).
					</p>
					<div class="bg-red-50 border-l-4 border-red-500 p-2 rounded-r-xl mb-6">
						<div class="flex gap-3">
							...
							<div class="text-sm text-red-800">
								...
								<p>
									Todos los clientes seleccionados serán eliminados
